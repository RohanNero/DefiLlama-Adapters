...
async function tvl(api) {
  ...
  for (const market of markets) {
    ...
    const tokensObject = await sui.getDynamicFieldObject(
      syTableParentId, syTokens, { idType: "0x1::type_name::TypeName" }
    );
    if (!tokensObject) continue;

    const syCoinType = "0x" + tokensObject.fields.value.fields.name;
    if (!watchCoinType.includes(syCoinType)) continue;

    // wip: missing PT
    api.add(syCoinType, fields.total_sy);
  }
}
