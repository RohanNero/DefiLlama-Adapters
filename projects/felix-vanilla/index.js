// Morpho Blue contract on Hyperliquid https://docs.morpho.org/get-started/resources/addresses#morpho-v1-contracts
const MORPHO_BLUE = "0x68e37dE8d93d3496ae143F2E900490f6280C57cD";

// Felix Vanilla market IDs from https://www.usefelix.xyz/vanilla/stats
const FELIX_VANILLA_MARKETS = [
...
const tvl = async (api) => {
  const calls = FELIX_VANILLA_MARKETS.map(id => ({ target: MORPHO_BLUE, params: [id] }));

  const [marketParams, marketStates] = await Promise.all([
    api.multiCall({ abi: abis.idToMarketParams, calls }),
    api.multiCall({ abi: abis.market, calls }),
  ]);

  for (let i = 0; i < FELIX_VANILLA_MARKETS.length; i++) {
    const { loanToken } = marketParams[i];
    const { totalSupplyAssets } = marketStates[i];

    if (BigInt(totalSupplyAssets) > 0n) {
      api.add(loanToken, totalSupplyAssets);
    }
  }
};
...
const borrowed = async (api) => {
  ...
  for (let i = 0; i < FELIX_VANILLA_MARKETS.length; i++) {
    const { loanToken } = marketParams[i];
    const { totalBorrowAssets } = marketStates[i];

    if (BigInt(totalBorrowAssets) > 0n) {
...
module.exports = {
  methodology:
    "Felix Vanilla TVL tracks the total supply assets (lender deposits) in Felix's vanilla lending markets.",
  doublecounted: true,
  hyperliquid: {
    tvl,
    borrowed,
