...
async function tvl(api) {
  ...
  // --- 5. Aave V3 supply positions on subvaults ---
  ...
  // --- 6. Lido withdrawal NFTs on subvaults ---
...
async function borrowed(api) {
  const subvaultCount = await api.call({ abi: "uint256:subvaults", target: VAULT });
  const svIndices = Array.from({ length: subvaultCount }, (_, i) => i);
  const subvaultAddrs = await api.multiCall({
    abi: "function subvaultAt(uint256) view returns (address)",
    target: VAULT,
...
module.exports = {
  doublecounted: true,
  methodology:
    "TVL is idle assets plus Spark and Aave supply positions plus pending Lido withdrawal NFTs. Borrowed shows debt from Spark (WETH) and Aave (USDe).",
